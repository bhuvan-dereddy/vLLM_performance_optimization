                ...
                with completed_requests_lock:
                    if num_completed_requests < max_num_completed_requests:
                        if num_output_tokens and num_output_tokens > 0:
                            ...
                            # Don't crash the whole run if one request returned no tokens.
                            request_metrics[common_metrics.INTER_TOKEN_LAT] = None
                            request_metrics[common_metrics.ERROR_CODE] = request_metrics.get(common_metrics.ERROR_CODE) or -1
                            request_metrics[common_metrics.ERROR_MSG] = request_metrics.get(common_metrics.ERROR_MSG) or "zero output tokens"
                        ...
                        e2e = request_metrics.get(common_metrics.E2E_LAT, 0) or 0
                        if e2e <= 0:
                            request_metrics[common_metrics.REQ_OUTPUT_THROUGHPUT] = 0
                        else:
                            request_metrics[common_metrics.REQ_OUTPUT_THROUGHPUT] = num_output_tokens / e2e
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seed", type=int, default=0, help="Random seed for deterministic request generation.")
