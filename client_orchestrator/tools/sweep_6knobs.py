...
def run_one(mask_bits):
    ...
    # Try to extract p95 end-to-end latency (seconds) from common llmperf summary layouts
    metric = None

    # Layout A: nested dict with quantiles
    e2e = summary.get("end_to_end_latency_s")
    if isinstance(e2e, dict):
        metric = e2e.get("p95")

    # Layout B: flattened keys (some variants)
    if metric is None:
        metric = summary.get("results_end_to_end_latency_s_quantiles_p95")

    # Layout C: if only a dict of quantiles exists under a different name
    if metric is None:
        e2e2 = summary.get("results_end_to_end_latency_s_quantiles")
        if isinstance(e2e2, dict):
            metric = e2e2.get("p95")

    # Keep these if you want them later (not used for ranking right now)
    metric_ttft = summary.get("results_ttft_s_quantiles_p95")
    metric_thr  = summary.get("results_mean_output_throughput_token_per_s")

    # Ensure metric is float if present
